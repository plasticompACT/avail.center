...
if(companyLicenseBtn){
  companyLicenseBtn.onclick=()=>{companyLicenseModal.classList.add("flex");companyLicenseModal.classList.remove("hidden");};
}
if(companyPlasticoCancel){
  companyPlasticoCancel.onclick=()=>{companyLicenseModal.classList.add("hidden");companyLicenseModal.classList.remove("flex");};
}


// === Company License: Redeem plastico code ===
function validateCompanyPlastico(code) {
  if (!/^\d{8}$/.test(code)) return false;
  const digits = code.split("").map(Number);
  const sum = digits.slice(0, 7).reduce((a, b) => a + b, 0);
  const checksum = sum % 10;
...
if (companyPlasticoVerify) {
  companyPlasticoVerify.onclick = async () => {
    const code = companyPlasticoInput.value.trim();
    if (!validateCompanyPlastico(code)) {
      companyPlasticoError.classList.remove("hidden");
      return;
    }
    companyPlasticoError.classList.add("hidden");

    try {
      const resp = await fetch("/.netlify/functions/generateCompanyLicense", {
        method: "POST"
      });
      if (!resp.ok) {
        alert("Error generating company license file");
        return;
      }
      ...
      const filename = m ? m[1] : "company-license.html";
      ...
      companyLicenseModal.classList.add("hidden");
      companyLicenseModal.classList.remove("flex");
